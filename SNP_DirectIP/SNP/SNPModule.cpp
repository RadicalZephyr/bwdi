...
{
  //------------------------------------------------------------------------------------------------------------------------------------
  Network<BinPeerID> *pluggedNetwork = NULL;
...
*/
  ...
  */
  //------------------------------------------------------------------------------------------------------------------------------------
  void passAdvertisement(const BinPeerID& host, Util::MemoryFrame ad)
  {
    INTERLOCKED;
    ...
    AdFile *adFile = NULL;
    {
      ...
      {
        // if peer IDs equal
        if(!memcmp(&currAd->gameInfo.peerID, &host, sizeof(BinPeerID)))
        {
          adFile = &(*currAd);
          break;
        }
    ...
    adFile->gameInfo.peerID = host;
    adFile->gameInfo.pExtra = adFile->extraBytes;
  }
  void removeAdvertisement(const BinPeerID& host)
  {
  }
  void passPacket(const BinPeerID& sender, Util::MemoryFrame packet)
  {
    INTERLOCKED;
...
//    DropMessage(0, "spiGetGameInfo");
  ...
  //------------------------------------------------------------------------------------------------------------------------------------
  bool __stdcall spiSend(DWORD addrCount, BinPeerID * *addrList, char *buf, DWORD bufLen)
  {
...
//      DropMessage(0, "S> %s", sprintfBytes(buf, bufLen));
  ...
  //------------------------------------------------------------------------------------------------------------------------------------
  bool __stdcall spiReceive(BinPeerID **senderPeer, char **data, DWORD *databytes)
  {
    INTERLOCKED;
...
//        DropMessage(0, "Received storm packet %d bytes", *databytes);
  ...
  //------------------------------------------------------------------------------------------------------------------------------------
  bool __stdcall spiFree(BinPeerID * addr, char *data, DWORD databytes)
  {
    INTERLOCKED;
...
//    DropMessage(0, "spiFree");
  ...
  //------------------------------------------------------------------------------------------------------------------------------------
  bool __stdcall spiCompareNetAddresses(BinPeerID * addr1, BinPeerID * addr2, DWORD *dwResult)
  {
    INTERLOCKED;
    ...
    *dwResult = (0 == memcmp(&addr1, &addr2, sizeof(BinPeerID)));
    return true;
  }
  //------------------------------------------------------------------------------------------------------------------------------------
...
//    DropMessage(0, "spiUnlockDeviceList");
  ...
  //------------------------------------------------------------------------------------------------------------------------------------
  bool __stdcall spiFreeExternalMessage(BinPeerID * addr, char *data, DWORD databytes)
  {
...
//
  ...
  //------------------------------------------------------------------------------------------------------------------------------------
  bool __stdcall spiReceiveExternalMessage(BinPeerID * *addr, char **data, DWORD *databytes)
  {
