#pragma once
/**
 *  Capsulates functionality:
 * - matches incoming packet id's
 * - casts incoming memory blocks to the identified packet types
 *
 *  Usage:
 *  handlers (callbacks) are pushed (addHandler)
 *  one handler is called when processing a packet (handlePacket)
 */
