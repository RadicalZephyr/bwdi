...
namespace Util
{
  ...
  Mutex::Mutex(const char* systemName, bool instantOwnership)
    : initialOwnership(false)
  {
	  hMutex = CreateMutex(NULL, instantOwnership, systemName);
    if(instantOwnership && GetLastError() != ERROR_ALREADY_EXISTS)
      initialOwnership = true;
  }
  ...
  void Mutex::releaseInitialOwnership()
  {
    if(initialOwnership)
    {
      initialOwnership = false;
