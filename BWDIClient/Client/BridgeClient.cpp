...
namespace BWDI
{
  ...
  //public:
    ...
    BWDI::StaticGameData* gameData;
    BWDI::DynamicGameData dynamicData;
    ...
    bool connect()
    {
      ...
      // receive handshake
      {
        ...
        if(handshake.serverVersion != SVN_REV)
        {
          ...
          throw GeneralException(__FUNCTION__ ": BWDI.dll("
            + Util::Strings::intToString(handshake.serverVersion)
            + ") and BWDIDriver.dll(" SVN_REV_STR ") version mismatch.");
        }
      }
...
//          packetSwitch.addHandler(sortEventKnownUnitAdd);
    ...
    bool pushDrawText(CoordinateBase base, Position pos, const char* text, int size)
    {
    ...
    bool pushDrawCircle(CoordinateBase base, Position center, float r, int color, bool solid)
    {
