...
namespace BWDI
{
  ...
  //public:
    //-------------------------- PUBLIC DATA ----------------------------------------------------
    BWDI::StaticGameData* gameData = NULL;
    //-------------------------- INIT -----------------------------------------------------------
    ...
    */
    ...
    void invokeOnFrame()
    {
      ...
      sharedStuff.staticData.get().unitsAdded.clear();
      sharedStuff.staticData.get().unitsRemoved.clear();
    }
    //------------------------------ ADD KNOWN UNIT ----------------------------------------------
    int addKnownUnit(UnitState **out_pKnownUnit, UnitAddEventTypeID reason)
    {
      // check prerequisites
      ...
      UnitState* knownUnit = &gameData->units.allocate(index);
      if(out_pKnownUnit)
        *out_pKnownUnit = knownUnit;

      // push known unit event
      BWDI::UnitAddEvent entry;
      entry.unitID = index;
      entry.type = reason;
      sharedStuff.staticData.get().unitsAdded.push_back(entry);
    ...
    void removeKnownUnit(int index, UnitRemoveEventTypeID reason)
    {
      ...
      // push known unit event
      BWDI::UnitRemoveEvent entry;
      entry.unitID = index;
      entry.type = reason;
      sharedStuff.staticData.get().unitsRemoved.push_back(entry);
    }
